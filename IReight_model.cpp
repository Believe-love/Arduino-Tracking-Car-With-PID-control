...
void Deal_Usart_AData(void) // 处理模拟值数据（优化内存版）
{
    // 使用静态局部变量，只分配一次，不占栈空间
    static char* strArray[8];      // 最多8个传感器
    static char* strArraytemp[2];
    static char str_temp[80];      // 实际数据包长度通常小于80
    static char mystr_temp[8][10];
    ...
    // 复制数据包到临时缓冲区（注意安全长度）
    int len = strlen((char*)new_package);
    if (len >= 80) len = 79; // 防止溢出
    strncpy(str_temp, (char*)new_package, len);
    str_temp[len] = '\0';
